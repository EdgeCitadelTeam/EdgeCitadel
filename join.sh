...
#
# Usage: ./join.sh <server-host> <mqtt-password>
#
# Auto-detects: agent ID (from hostname), display name, role,
# device type, gateway URL, gateway token.
# ═══════════════════════════════════════════════════════════════
...
if [[ -z "$CITADEL_HOST" || -z "$MQTT_PASS" ]]; then
    echo "Usage: ./join.sh <server-host> <mqtt-password>"
    echo ""
    ...
    echo ""
    echo "Example: ./join.sh 100.97.29.74 xK9mPv2Ld8nQwR3s"
    exit 1
fi
...
# Agent ID: hostname, lowercased, sanitized
