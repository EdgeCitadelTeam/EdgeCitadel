...
def _controller(tmp_path: Path) -> ControllerConfig:
    ...
    return ControllerConfig(
        ...
        compose_project="edgecitadel-artifact-ec-lab-01", bind_host="127.0.0.1", advertised_host="127.0.0.1",
        advertised_ip="127.0.0.1", app_url="http://127.0.0.1:18080", agg_url="http://127.0.0.1:18080",
        nats_url="nats://127.0.0.1:14222", monitor_url="http://127.0.0.1:18222",
        inventory_url="http://127.0.0.1:18080/api/lab/status", controller_machine_id_sha256="a" * 64,
        source_commit="d" * 40, source_snapshot_sha256="e" * 64,
        credential_sha256=credential_sha256(credential), credential_file=credential, fixture_image_id="sha256:" + "c" * 64,
        state_dir=tmp_path / "controller-state", evidence_dir=tmp_path / "evidence",
    )


def test_fixture_config_keeps_no_crash_as_json_null() -> None:
    config = build_fixture_config(run_id="ec-lab-01", agent_id="fixture-1", behavior="echo", delay_ms=125, crash_point=None)
    assert json.loads(json.dumps(config.__dict__))["crash_point"] is None
    assert config.outcome_db == "/run/state/outcomes.sqlite"


def test_fixture_create_argv_uses_only_immutable_image_and_secret_file(tmp_path: Path) -> None:
    controller = _controller(tmp_path)
    node_state = tmp_path / "node-state"
    config = node_state / "native-control.json"
    argv = build_fixture_create_argv(
        controller=controller, credential_file=controller.credential_file, node_state_dir=node_state,
        config_path=config, container_name="edgecitadel-node-ec-lab-01--fixture-1",
    )
...
def test_start_and_stop_persist_reservation_bound_node_state(tmp_path: Path, monkeypatch) -> None:
    controller = _controller(tmp_path)
    ...
    monkeypatch.setattr("scripts.research.lab_node._machine_id_sha256", lambda: "a" * 64)
    ...
    arguments = Namespace(
        controller_config=controller_path, credential_file=controller.credential_file,
        host_id="controller-lab-01", agent_id="fixture-1", behavior="echo", delay_ms=0,
        crash_point=None, state_root=tmp_path / "node-state",
    )
    ...
    assert inventory_calls == [("POST", "/reservations"), ("DELETE", "/reservations/fixture-1")]


def test_stop_retries_after_remote_failure_when_container_is_already_absent(tmp_path: Path, monkeypatch) -> None:
    controller = _controller(tmp_path)
    controller_path = tmp_path / "controller.json"
    write_private_json(controller_path, controller.to_dict())
    state_file = tmp_path / "node-state/ec-lab-01/ec-lab-01--fixture-1/node-state.json"
    state = NodeState(
        ...
        agent_id="fixture-1", qualified_agent_id="ec-lab-01--fixture-1", reservation_id="reservation-1",
        declared_host_id="controller-lab-01", machine_id_sha256="a" * 64, container_id="container-1",
        container_name="edgecitadel-node-ec-lab-01--fixture-1", fixture_image_id=controller.fixture_image_id,
        config_path=state_file.parent / "native-control.json", state_dir=state_file.parent,
        log_path=state_file.parent / "fixture.log", reservation_state="active", started_at="2026-07-27T00:00:00Z",
    )
    ...
    def runner(argv, **_kwargs):
        ...
        return subprocess.CompletedProcess(
            argv, 1, "", f"Error response from daemon: No such container: {state.container_name}\n"
        )
    ...
    arguments = Namespace(
        controller_config=controller_path, credential_file=controller.credential_file,
        agent_id="fixture-1", state_root=tmp_path / "node-state", retain_reservation=True,
    )
...
def test_stop_does_not_hide_unexpected_docker_removal_failure(tmp_path: Path, monkeypatch) -> None:
    controller = _controller(tmp_path)
    controller_path = tmp_path / "controller.json"
    write_private_json(controller_path, controller.to_dict())
    state_file = tmp_path / "node-state/ec-lab-01/ec-lab-01--fixture-1/node-state.json"
    ...
    arguments = Namespace(
        controller_config=controller_path, credential_file=controller.credential_file,
        agent_id="fixture-1", state_root=tmp_path / "node-state", retain_reservation=False,
    )
...
def test_inventory_preserves_only_known_safe_conflict_detail(tmp_path: Path, monkeypatch) -> None:
    ...
    def conflict(*_args, **_kwargs):
        raise urllib.error.HTTPError(
            controller.inventory_url, 409, "Conflict", {},
    ...
    with pytest.raises(ValueError, match="agent_id has an active reservation"):
        _inventory_request(controller, controller.credential_file, "POST", "/reservations", {})


def test_start_rejects_remote_loopback_before_inventory_or_docker(tmp_path: Path, monkeypatch) -> None:
    controller = _controller(tmp_path)
    controller_path = tmp_path / "controller.json"
    write_private_json(controller_path, controller.to_dict())
    monkeypatch.setattr("scripts.research.lab_node._machine_id_sha256", lambda: "d" * 64)
    monkeypatch.setattr("scripts.research.lab_node._inventory_request", lambda *_: (_ for _ in ()).throw(AssertionError("inventory")))
    arguments = Namespace(
        controller_config=controller_path, credential_file=controller.credential_file,
        host_id="gateway-lab-02", agent_id="fixture-1", behavior="echo", delay_ms=0,
        crash_point=None, state_root=tmp_path / "node-state",
    )
    try:
...
def test_doctor_reports_safe_remote_target_without_reserving_or_creating(tmp_path: Path, monkeypatch) -> None:
    controller = _controller(tmp_path)
    ...
    monkeypatch.setattr("scripts.research.lab_node._machine_id_sha256", lambda: "d" * 64)
    monkeypatch.setattr("scripts.research.lab_node.shutil.which", lambda command: "/usr/bin/docker" if command == "docker" else None)
    monkeypatch.setattr("scripts.research.lab_node.subprocess.run", runner)
    monkeypatch.setattr(
        "scripts.research.lab_node._inventory_request",
    ...
    report = doctor_node(
        Namespace(controller_config=controller_path, credential_file=controller.credential_file)
    )
...
def test_doctor_publish_binds_active_node_to_source_and_route_facts(tmp_path: Path, monkeypatch) -> None:
    controller = replace(
    ...
    state = NodeState(
        schema_version="lab-node-state.v1", phase="active", run_id=controller.run_id,
        agent_id="fixture-1", qualified_agent_id="ec-lab-01--fixture-1", reservation_id="reservation-1",
        declared_host_id="gateway-lab-02", machine_id_sha256="d" * 64, container_id="container-1",
        container_name="edgecitadel-node-ec-lab-01--fixture-1", fixture_image_id=controller.fixture_image_id,
        config_path=state_file.parent / "native-control.json", state_dir=state_file.parent,
        log_path=state_file.parent / "fixture.log", reservation_state="active", started_at="2026-07-27T00:00:00Z",
    )
    ...
    monkeypatch.setattr("scripts.research.lab_node._machine_id_sha256", lambda: "d" * 64)
    monkeypatch.setattr("scripts.research.lab_node.shutil.which", lambda _: "/usr/bin/docker")
    monkeypatch.setattr("scripts.research.lab_node.subprocess.run", lambda argv, **_kwargs: subprocess.CompletedProcess(argv, 0, "", ""))
    monkeypatch.setattr("scripts.research.lab_node._inventory_request", inventory)
    monkeypatch.setattr("scripts.research.lab_node._os_release", lambda: "Ubuntu 24.04.1 LTS")
    monkeypatch.setattr("scripts.research.lab_node.socket.gethostname", lambda: "gateway-lab-02")
    ...
    monkeypatch.setattr(
        ...
        lambda _: SourceProvenance(controller.source_commit, False, controller.source_snapshot_sha256, "f" * 64),
    )
    monkeypatch.setattr(
        "scripts.research.lab_node._network_path",
        lambda _: {
            "source_ip": "192.0.2.45", "destination_ip": "192.0.2.44", "interface": "eth0",
            "route_output_sha256": "a" * 64, "controller_dns_name": "controller.research.test",
        },
    )

    report = doctor_node(Namespace(
        controller_config=controller_path, credential_file=controller.credential_file,
        agent_id="fixture-1", host_id="gateway-lab-02", state_root=tmp_path / "node-state", publish=True,
    ))
