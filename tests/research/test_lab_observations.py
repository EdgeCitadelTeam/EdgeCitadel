...
def test_append_observation_assigns_ordered_sequences_and_explicit_nulls(
    tmp_path: Path,
) -> None:
...
def test_append_observation_rejects_secrets_and_absolute_paths(
    tmp_path: Path, data: dict[str, object]
) -> None:
    with pytest.raises(LabConfigError):
        append_observation(
            tmp_path / "lab-observations.jsonl",
            {
                "event": "bad",
                "agent_id": None,
                "reservation_id": None,
