...
def test_command_wait_writes_one_exact_terminal_and_observation_sequence(
    tmp_path: Path,
) -> None:
    ...
    observations = [
        json.loads(line)
        for line in (config.evidence_dir / "lab-observations.jsonl")
        .read_text()
        .splitlines()
    ]
    assert [row["event"] for row in observations] == [
        "command.accepted",
        "command.wire_submitted",
        "command.terminal",
    ]
...
def test_await_retries_network_and_server_failures_to_completion(
    tmp_path: Path,
) -> None:
    state_file, _ = _state(tmp_path)
    task_id = "10000000-0000-4000-8000-000000000001"
...
def test_two_wire_copies_are_identical_and_header_free(tmp_path: Path) -> None:
    ...
    result = command_run(
        ...
        wire_copies=2,
        opener=opener,
        wire_publisher=lambda subject, payload, headers: published.append(
    ...
    observations = [
        json.loads(line)
        for line in (
            state_file.parent.parent.parent / "evidence/lab-observations.jsonl"
        )
        .read_text()
        .splitlines()
    ]
...
def test_duplicate_publish_preserves_primary_error_when_drain_also_fails(
    tmp_path: Path,
) -> None:
    state_file, _ = _state(tmp_path)
...
def test_commands_fail_closed_on_agent_http_terminal_and_queue_errors(
    tmp_path: Path,
) -> None:
    for label, messages, queue, match in (
        (
            "conflict",
            _messages() + [{**_messages("wrong", terminal_id="result-2")[-1]}],
            {"pending": 0, "ack_pending": 0},
            "terminal",
        ),
        (
            "identity",
            [*_messages()[:-1], {**_messages()[-1], "recipient_id": "other"}],
            {"pending": 0, "ack_pending": 0},
            "identity",
        ),
    ...
    for label, replies, match in (
        ...
        with pytest.raises(LabConfigError, match=match):
            command_run(
                ...
                root / "failed.json",
                wait=False,
                opener=_Replies(replies),
            )
        assert not (root / "failed.json").exists()


def test_queued_reconnect_requires_same_reservation_and_strict_order(
    tmp_path: Path,
) -> None:
    state_file, _ = _state(tmp_path)
    task_id = "10000000-0000-4000-8000-000000000001"
                    ...
                    _inventory(
                        events=[
                            ...
                            },
                            {
                                ...
                                "reservation_id": "reservation-1",
                                "event": "resumed",
                                "observed_at": "2026-07-27T00:00:02.000Z",
                            },
                        ]
                    ),
                ),
...
def test_queued_reconnect_retries_transient_inventory_failures_to_deadline(
    tmp_path: Path,
) -> None:
    state_file, _ = _state(tmp_path)
    task_id = "10000000-0000-4000-8000-000000000001"
...
def test_finalized_bundle_and_existing_results_fail_before_io(tmp_path: Path) -> None:
    ...
    command_argv = [
        *prefix,
        ...
        "--no-wait",
        "--result-file",
        str(cli_root / "cli-command.json"),
    ]
    completed = subprocess.run(
        command_argv, cwd=repo_root, text=True, capture_output=True
    )
...
def test_relocate_slice2_media_replaces_report_and_validates_correlation(
    ...
    def copy_media(_root: Path, target: Path, results: object) -> dict[str, object]:
        ...
        for index, project in enumerate(("desktop", "mobile"), start=1):
            ...
            (media / "operator-metadata.json").write_text(
                json.dumps(
                    {
                        "project": project,
                        "task_id": task_id,
            ...
            (api / "system-status.json").write_text(
                json.dumps({"nats_connected": True, "jetstream_stream_ok": True})
            )
            (api / "registry.json").write_text(
                json.dumps(
                    [
                        {
                            "agent_id": "shell-1",
                            "agent_state": "online",
                            "card": {"metadata": {"runtime.conformance": "L1"}},
                        }
                    ]
                )
            )
            (api / "messages.json").write_text(
                json.dumps(
                    [
                        {
                            "id": f"command-{index}",
            ...
            (api / "queue.json").write_text(
                json.dumps({"pending": 0, "ack_pending": 0})
            )
        return portable
    ...
    portable["projects"]["desktop"]["attachments"][0]["path"] = (
        "raw/playwright/mobile/chat.png"
    )
    ...
    portable["projects"]["desktop"]["attachments"][0]["path"] = (
        "raw/playwright/desktop/chat.png"
    )
        ...
        "projects": {
            "desktop": {
                "attachments": [
                    {
                        "name": "chat",
                        "path": "raw/playwright/desktop/chat.png",
...
def test_live_consumers_use_only_agent_inbox_jsz_rows(monkeypatch) -> None:
    ...
    assert subjects == frozenset(
        row["config"]["filter_subject"] for row in expected_rows
    )


def test_gate_cleanup_stops_controller_from_state_when_config_disappears(
    tmp_path: Path, monkeypatch
) -> None:
    from scripts.research import lab_gate
    ...
    for entrypoint in (lab_gate.run_two_node_lifecycle, lab_gate.run_operator_journey):
        ...
        def runner(argv, **_kwargs):
            ...
            if "lab_controller.py" in " ".join(argv) and "start" in argv:
                state_file = (
                    tmp_path / "tmp/research/lab/ec-lab-01/controller-state.json"
                )
                state_file.parent.mkdir(parents=True, exist_ok=True)
                state_file.write_text("{}\n")
            if "stop" in argv:
                return subprocess.CompletedProcess(
                    argv, 0, '{"owned_resources_removed":true}\n', ""
                )
        ...
        with pytest.raises(BaseException):
            entrypoint(
                repo_root=tmp_path, run_id="ec-lab-01", host_id="controller-lab-01"
            )
        controller_stops = [
            argv
            for argv in calls
            if "lab_controller.py" in " ".join(argv) and "stop" in argv
        ]
        assert len(controller_stops) == 1
        assert controller_stops[0][-2:] == [
            "--state-file",
            str(tmp_path / "tmp/research/lab/ec-lab-01/controller-state.json"),
        ]
