...
def test_trace_import_invokes_owned_runtime_with_archive_and_source(
    tmp_path, monkeypatch
):
    ...
    calls = []
    monkeypatch.setattr(cli, "_start_agentd", lambda state: None)
    monkeypatch.setattr(cli, "_toolkit_python", lambda state: Path("/owned/python"))
