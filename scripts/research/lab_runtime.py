...
def _git(repo_root: Path, *argv: str) -> str:
    return subprocess.run(
        ["git", *argv], cwd=repo_root, check=True, capture_output=True, text=True
    ).stdout
...
def build_fixture_image(
    ...
    runner(
        [
            ...
            "--label",
            f"ai.edgecitadel.run-id={validated_run_id}",
            "--tag",
            tag,
            str(root),
        ],
    ...
    try:
        completed = runner(
            ["docker", "image", "inspect", "--format={{.Id}}", tag], cwd=root
        )
        image_id = completed.stdout.strip()
        if (
            not image_id.startswith(IMAGE_ID_PREFIX)
            or len(image_id) != len(IMAGE_ID_PREFIX) + 64
            or any(
                c not in "0123456789abcdef" for c in image_id[len(IMAGE_ID_PREFIX) :]
            )
        ):
