...
def write_controller_state(state_file: Path, state: ControllerOwnershipState) -> None:
    ...
    encoded = json.dumps(_state_dict(state), sort_keys=True, separators=(",", ":"), allow_nan=False).encode() + b"\n"
...
def load_controller_state(state_file: Path) -> ControllerOwnershipState:
    ...
    if not isinstance(value, dict) or value.get("schema_version") != "lab-controller-state.v1":
        raise LabConfigError("controller state is invalid")
    phase = value.get("phase")
    if phase not in _PHASES:
        raise LabConfigError("controller state phase is invalid")
    try:
        resources = tuple(OwnedResource(**resource) for resource in value["owned_resources"])
        environment = value["compose_environment"]
        if not isinstance(environment, dict) or not all(isinstance(key, str) and isinstance(item, str) for key, item in environment.items()):
            raise LabConfigError("controller state environment is invalid")
        return ControllerOwnershipState(
            schema_version="lab-controller-state.v1", phase=phase, config=_config(value["config"]),
            compose_file=_path(value["compose_file"], "compose_file"), compose_environment=environment,
            artifact_scratch_root=_path(value["artifact_scratch_root"], "artifact_scratch_root"),
            raw_credential_file=_path(value["raw_credential_file"], "raw_credential_file"),
            service_env_file=_path(value["service_env_file"], "service_env_file"), owned_resources=resources,
            completed_cleanup_steps=tuple(str(item) for item in value["completed_cleanup_steps"]),
            exported_image_paths=tuple(_path(item, "exported_image_path") for item in value["exported_image_paths"]),
            controller_argv=tuple(str(item) for item in value["controller_argv"]), started_at=str(value["started_at"]),
        )
    except (KeyError, TypeError, LabConfigError) as error:
...
def _validate_start_network(args: argparse.Namespace) -> tuple[str, str, str, str, str]:
    ...
    ports = tuple(getattr(args, name, None) for name in (
        "http_port", "nats_port", "monitor_port"
    ))
    if bind.is_loopback:
        for value in ports:
        ...
        if not getattr(args, "trusted_network_confirm", False):
            raise LabConfigError("non-loopback binding requires trusted network confirmation")
...
def _validate_nats_configuration(
    ...
    *,
    ...
    _write_evidence_json(
        ...
        {"config_sha256": sha256_file(nats_config), "exit_status": completed.returncode},
    )
    if completed.returncode != 0:
...
def _port(project: str, compose_file: Path, service: str, container_port: int) -> int:
    completed = subprocess.run(
        ["docker", "compose", "--project-name", project, "--file", str(compose_file), "port", service, str(container_port)],
...
def _require_system_health(url: str) -> None:
    ...
    for _ in range(30):
        try:
            with urllib.request.urlopen(f"{url}/api/system/status", timeout=2) as response:
...
def _write_evidence_json(path: Path, value: object) -> None:
    if path.exists():
        try:
            ...
            raise LabConfigError(f"retained evidence is invalid: {path.name}") from error
        if existing != value:
...
def _portable_value(state: ControllerOwnershipState, value: object) -> object:
    ...
    if isinstance(value, Mapping):
        return {
            str(key): _portable_value(state, item)
...
def _controller_command_snapshot(
    ...
    if command_root.is_dir():
        for path in sorted(command_root.glob("*.json")):
            try:
                ...
                raise LabConfigError("controller command evidence is invalid") from error
    ...
    for command in commands.values():
        ...
        retained = [item.get("observed_at") for item in matching if item.get("event") == "retained"]
        resumed = [item.get("observed_at") for item in matching if item.get("event") == "resumed"]
        if (
    ...
    *,
    ...
    if observation_snapshot.is_file():
        ...
        try:
            ...
            raise LabConfigError("controller observation journal is unavailable") from error
...
def _docker_inventory(runner: object) -> frozenset[OwnedResource]:
    ...
    for kind, argv in queries:
        ...
        resources.update(
            OwnedResource(cast(Literal["container", "network", "volume", "image"], kind), name)
...
def _load_json_evidence(path: Path) -> object:
    try:
        ...
        raise LabConfigError(f"required evidence is unavailable: {path.name}") from error
...
def _write_task6_evidence(
    bundle: Path, manifest: Mapping[str, object]
) -> None:
...
def _build_lab_manifest(
    ...
    *,
    ...
    manifest: dict[str, object] = {
        ...
        "command": [
            cast(list[str], _portable_value(state, state.controller_argv))
        ],
        "timing": {
            "started_at": state.started_at,
...
def _render_compose(
    ...
    *,
    ...
    completed = runner(  # type: ignore[operator]
        [
            "docker", "compose", "--project-name", state.config.compose_project,
            "--file", str(state.compose_file), "config", "--no-env-resolution",
    ...
    *,
    ...
    try:
        ...
        image_remaining = tuple(
            resource for resource in failed.owned_resources
            if resource.kind == "image"
        )
        cleanup_errors.append(f"image cleanup failed: {error!r}")
    ...
    if complete and failed.config.evidence_dir.is_dir():
        try:
            invalid_manifest = _invalid_start_manifest(
                failed, cleanup, dependencies
            )
            ...
            if result != "INVALID" or not (
                failed.config.evidence_dir / "manifest.json"
            ).is_file():
...
def start_controller(args: argparse.Namespace) -> ControllerConfig:
    ...
    try:
        ...
        toolchain = json.loads((repo_root / "scripts/research/toolchain.json").read_text())
        ...
        placeholder = ControllerConfig(
            run_id=run_id, lab_variant=args.lab_variant, controller_host_id=host_id,
            compose_project=environment.project, bind_host=bind_host,
            advertised_host=str(args.advertise_host), advertised_ip=advertised_ip,
            app_url="", agg_url="", nats_url="", monitor_url="", inventory_url="",
            controller_machine_id_sha256=machine_id_sha256,
            source_commit=source.commit, source_snapshot_sha256=source.source_snapshot_sha256,
            credential_sha256=token_hash, credential_file=environment.credential_file,
        ...
        compose_environment = {
            "LAB_RUN_ID": run_id, "LAB_BIND_HOST": bind_host,
            "LAB_HTTP_PORT": http_requested, "LAB_NATS_PORT": nats_requested,
            "LAB_MONITOR_PORT": monitor_requested,
            "LAB_SERVICE_ENV_FILE": str(service_env), "LAB_NATS_CONFIG": str(nats_config),
            "LAB_DATA_DIR": str(data_dir), "LAB_TOKEN_SHA256": token_hash,
            "LAB_NATS_IMAGE": nats_image, "LAB_AGGREGATOR_IMAGE": aggregator_tag,
            "LAB_DASHBOARD_IMAGE": dashboard_tag, "LAB_NGINX_IMAGE": LAB_NGINX_IMAGE,
        }
        state = ControllerOwnershipState(
            schema_version="lab-controller-state.v1", phase="starting", config=placeholder,
            compose_file=compose_file, compose_environment=compose_environment,
            artifact_scratch_root=scratch_root,
            raw_credential_file=environment.credential_file,
            service_env_file=service_env, owned_resources=(), completed_cleanup_steps=(), exported_image_paths=(),
            controller_argv=(
                "scripts/research/lab_controller.py", "start", "--run-id", run_id,
                "--host-id", host_id, "--lab-variant", str(args.lab_variant),
                "--bind-host", bind_host, "--advertise-host", str(args.advertise_host),
            ),
            started_at=datetime.now(UTC).isoformat().replace("+00:00", "Z"),
        )
        write_controller_state(state_file, state)
        evidence_dir.mkdir(parents=True, mode=0o700)
        _write_evidence_json(evidence_dir / "controller-facts.json", {
            "source": {
        ...
        subprocess.run(
            [
                "docker", "compose", "--project-name", environment.project,
                "--file", str(compose_file), "build", "aggregator", "dashboard",
            ],
            cwd=repo_root,
        ...
        _write_evidence_json(evidence_dir / "controller-start.json", {
            "source": {
                "commit": source.commit,
                "dirty": source.dirty,
                "source_snapshot_sha256": source.source_snapshot_sha256,
        ...
        append_observation(evidence_dir / "lab-observations.jsonl", {
            "event": "controller-starting", "agent_id": None,
            "reservation_id": None, "task_id": None,
            "data": {"run_id": run_id, "host_id": host_id},
        })
        environment.start_topology(compose_file, compose_environment)
        http_port = int(http_requested) if http_requested else _port(environment.project, compose_file, "nginx", 80)
        nats_port = int(nats_requested) if nats_requested else _port(environment.project, compose_file, "nats", 4222)
        monitor_port = int(monitor_requested) if monitor_requested else _port(environment.project, compose_file, "nats", 8222)
        ...
        resources = tuple(dict.fromkeys((*environment.owned_resources(), *state.owned_resources)))
...
def stop_controller(
    ...
    *,
    ...
    failed_start_recovery = (
        cleanup_file.is_file()
        and not manifest_file.is_file()
    )
    ...
    if manifest_file.is_file() and "manifest-finalized" not in state.completed_cleanup_steps:
    ...
    run_scratch, artifact_state, raw_credential, owner_record = (
        _artifact_cleanup_paths(state)
    )
...
def export_fixture_image(
    ...
    *,
    ...
    if (
        ...
        or len(image_id) != 71
        or any(character not in "0123456789abcdef" for character in image_id.removeprefix("sha256:"))
    ):
    ...
    try:
        ...
        write_controller_state(
            state_file,
            replace(state, exported_image_paths=state.exported_image_paths + (output_path,)),
        )
        return result
    except BaseException:
        temporary.unlink(missing_ok=True)
...
def submit_command(
    ...
    *,
    ...
    result = {
        "run_id": state.config.run_id, "agent_id": agent_id, "task_id": task_id,
        "wire_copies": 1, "accepted_at": accepted_at, "terminal_at": None,
        "expected_output": expected_output, "status": "accepted",
    }
...
def await_command(
    ...
    *,
    ...
    state = load_controller_state(state_file)
    if state.phase != "active" or not agent_id or not task_id or not expected_output or result_file.exists():
        raise LabConfigError("await inputs are invalid")
    try:
        with opener(f"{state.config.agg_url}/api/messages?task_id={task_id}", timeout=10) as response:
            if response.status != 200:
                raise LabConfigError("task messages are unavailable")
            messages = json.loads(response.read())
        with opener(f"{state.config.agg_url}/api/agents/{agent_id}/queue", timeout=10) as response:
            if response.status != 200:
    ...
    terminals = [
        ...
        and item.get("type") == "result" and item.get("task_state") == "completed"
    ...
    if not isinstance(queue, dict) or queue.get("pending") != 0 or queue.get("ack_pending") != 0:
    ...
    result = {
        "run_id": state.config.run_id, "agent_id": agent_id, "task_id": task_id,
        "expected_output": expected_output, "terminal_at": terminal_at, "status": "completed",
    }
    write_private_json(result_file.resolve(), result)
    return result


def _active_command_state(state_file: Path, result_file: Path) -> ControllerOwnershipState:
...
def _accepted_observation(
    ...
    *,
    ...
    append_observation(state.config.evidence_dir / "lab-observations.jsonl", {
...
def _accepted_record(
    ...
    if (
        ...
        raise LabConfigError("command acceptance observation is invalid")
    return (
        agent_id, reservation_id, int(wire_copies), accepted_at,
        expected_output, request_body_sha256,
    )
...
def _terminal_receipt(
    ...
    command_identities = {
        (
            item.get("sender_id"), item.get("recipient_id"), item.get("task_id"),
            item.get("context_id"), item.get("hop_count"),
...
def _inventory_events(
    ...
    if not isinstance(inventory, Mapping) or not isinstance(inventory.get("reservation_events"), list):
        raise LabConfigError("reservation event snapshot is invalid")
    return tuple(
        item for item in inventory["reservation_events"]
...
def _await_run_command_reserved(
    ...
    *,
    ...
    if not task_id or not expected_output or qualification_kind not in {None, "queued-reconnect"}:
        raise LabConfigError("await inputs are invalid")
    (
        agent_id, reservation_id, wire_copies, accepted_at, recorded_output,
        request_body_sha256,
    ...
    while True:
        ...
        (
            ...
            messages, queue, agent_id, task_id, expected_output,
            request_body_sha256, wire_copies,
        )
        if complete:
            break
        remaining = deadline - time.monotonic()
...
def main() -> int:
    ...
    start.add_argument("--lab-variant", choices=("lifecycle", "operator-smoke", "operator-evidence"), required=True)
    ...
    start.add_argument("--state-root", type=Path, default=_repo_root() / "tmp/research/lab")
    ...
    status.add_argument("--state-root", type=Path, default=_repo_root() / "tmp/research/lab")
    ...
    stop.add_argument("--state-root", type=Path, default=_repo_root() / "tmp/research/lab")
    ...
    export.add_argument("--state-root", type=Path, default=_repo_root() / "tmp/research/lab")
    ...
    command.add_argument("--state-root", type=Path, default=_repo_root() / "tmp/research/lab")
    ...
    await_parser.add_argument("--state-root", type=Path, default=_repo_root() / "tmp/research/lab")
    ...
    qualify.add_argument("--state-root", type=Path, default=_repo_root() / "tmp/research/lab")
    ...
    if arguments.command == "export-image":
        print(json.dumps(export_fixture_image(state_file, arguments.output, arguments.result_file), sort_keys=True))
        return 0
    if arguments.command == "command":
        print(json.dumps(
            command_run(
                state_file, arguments.agent_id, arguments.body,
                arguments.expected_output, arguments.result_file,
                wait=arguments.wait, wire_copies=arguments.wire_copies,
            ),
            sort_keys=True,
        ))
        return 0
    if arguments.command == "await":
        print(json.dumps(
            await_run_command(
                state_file, arguments.task_id, arguments.expected_output,
                arguments.result_file, qualification_kind=arguments.qualification_kind,
            ),
            sort_keys=True,
        ))
        return 0
...
__all__ = [
    "ControllerOwnershipState", "await_command", "await_run_command", "command_run",
    "export_fixture_image", "load_controller_state", "qualify_controller",
    "start_controller", "stop_controller",
    "run_controller_preflight", "submit_command", "write_controller_state",
]
