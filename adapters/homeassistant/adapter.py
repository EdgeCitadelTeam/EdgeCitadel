...
class HomeAssistantWorker:
    def __init__(self, client: HomeAssistantClient, *, allowed_lights: set[str],
                 allowed_entities: set[str], allowed_cameras: set[str],
                 camera_rois: dict[str, list[int]], max_steps: int = DEFAULT_MAX_STEPS):
        self.client = client
    ...
    def _set_light(self, args: dict) -> dict:
        ...
        brightness = _int_arg(args.get("brightness", 255), "brightness",
    ...
    def _wait_state(self, args: dict) -> dict:
        ...
        timeout = _float_arg(args.get("timeout_sec", DEFAULT_MAX_WAIT_SEC),
                             "timeout_sec", minimum=0.01, maximum=DEFAULT_MAX_WAIT_SEC)
        poll = _float_arg(args.get("poll_sec", 0.1), "poll_sec",
    ...
    def sequence(self, steps: list[dict], *, restore: bool) -> dict:
        ...
        try:
            for index, step in enumerate(steps):
                ...
                results.append({"index": index, "operation": operation,
            ...
            if restore:
                for entity_id, original in originals.items():
                    ...
                    if state in {"on", "off"}:
                        brightness = original.get("attributes", {}).get("brightness") or 255
                        try:
                            self.client.set_light(entity_id, state, brightness)
        ...
        return {"steps": results, "restored_entities": sorted(originals) if restore else [],
...
def _load_worker() -> HomeAssistantWorker:
    ...
    for item in _csv_env("HA_CAMERA_ROIS"):
        ...
        if len(values) != 4:
            raise ValueError("HA_CAMERA_ROIS entries must be entity:x:y:w:h")
        rois[entity] = [_int_arg(v, "camera ROI", minimum=0, maximum=10000) for v in values]
    return HomeAssistantWorker(
        ...
        camera_rois=rois,
        max_steps=_int_arg(os.environ.get("HA_MAX_SEQUENCE_STEPS", DEFAULT_MAX_STEPS),
                           "HA_MAX_SEQUENCE_STEPS", minimum=1, maximum=100),
    )
