"""Adapter-side wrappers around the aggregator's memory.turns.* NATS API.
...
async def fetch_memory(nc: NATS, *, context_id: Optional[str], agent_id: str,
    ...
    if not context_id:
        return []
    payload = json.dumps({"context_id": context_id, "agent_id": agent_id,
                          "token_budget": token_budget}).encode()
    try:
        msg = await nc.request("memory.turns.get", payload, timeout=timeout)
    except (asyncio.TimeoutError, Exception) as e:  # noqa: BLE001
        log.warning("memory.turns.get failed (%s); proceeding stateless",
                    type(e).__name__)
        return []
    try:
...
async def persist_memory(nc: NATS, *, context_id: Optional[str], agent_id: str,
    ...
    try:
        ...
        log.warning("memory.turns.put failed (%s); turn not persisted",
                    type(e).__name__)
