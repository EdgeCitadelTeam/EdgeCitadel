...
This installable example is validation-only. It demonstrates the EdgeCitadel
Agent Package contract and is not a working agent runtime.
...
The commands below assume an activated `agent-platform` virtual environment. From `agent-platform/`, create and install it with:
...
From `agent-platform/`, `lock` validates package metadata and writes or regenerates `plugin.lock.json`. The `validate` command verifies the existing lock and emits deterministic inventory data; `validate` is read-only. Neither command imports or executes the runtime.
