...
export function parseTraceRoute(hash) {
  ...
  const run = params.get('run'), task = params.get('task'), step = params.get('step'), at = params.get('at'), event = params.get('event')
  ...
  route.invalid = ['run', 'task', 'step', 'at', 'event'].some(key => params.has(key) && route[key] === null)
  return route
}
...
export function navigateTrace(route = {}, { replace = false } = {}) {
  ...
  for (const key of ['run', 'task', 'step', 'at', 'event']) if (route[key]) params.set(key, route[key])
