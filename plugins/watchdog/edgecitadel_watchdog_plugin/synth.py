...
async def publish_synth(js: object, nc: NATS, env: dict) -> None:
    ...
    await js.publish(f"agents.{env['recipient_id']}.inbox", data, headers=headers)
    try:
        await nc.publish(f"agents.{WATCHDOG_AGENT_ID}.outbox", data)
    except Exception as e:  # noqa: BLE001
        log.warning("synth outbox mirror failed (%s): %s", type(e).__name__, e)
