...
PACKAGE_ROOT = Path(__file__).parents[2] / "agent-packages" / "examples" / "placeholder"
...
def test_repository_placeholder_package_is_valid() -> None:
    ...
    assert package.manifest == {
        ...
        "metadata": {
            ...
            "description": "Demonstrates the EdgeCitadel Agent Package contract.",
            "version": "0.2.0",
            "publisher": "local",
        },
        "compatibility": {
...
def test_placeholder_skill_contains_the_bounded_portable_procedure() -> None:
    assert (SKILL_ROOT / "SKILL.md").read_text(encoding="utf-8") == (
        "---\n"
        "name: placeholder\n"
        "description: Validate the EdgeCitadel Agent Package path without "
