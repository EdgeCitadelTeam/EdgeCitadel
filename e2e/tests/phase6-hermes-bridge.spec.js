...
// Skipped automatically if hermes-1 is not registered within 10s.

import { test, expect } from '@playwright/test';

const HERMES_AGENT_ID = 'hermes-1';
...
test.describe('Phase 6 — Hermes bridge', () => {
  ...
  test('hermes-1 registers with bridge runtime kind', async ({ request }) => {
    const card = await waitForAgent(request, HERMES_AGENT_ID);
    test.skip(!card, 'hermes-1 not online — install hermes locally and start the bridge adapter to run this spec');
  ...
  test('command roundtrip yields a non-empty result envelope', async ({ request }) => {
    const card = await waitForAgent(request, HERMES_AGENT_ID);
    test.skip(!card, 'hermes-1 not online');
    ...
    const resp = await request.post('/api/agents/hermes-1/dispatch', {
  ...
  test('multi-turn within a context_id shows continuity (Hermes upstream memory)',
    async ({ request }) => {
      ...
      const turn1 = await request.post('/api/agents/hermes-1/dispatch',
        { data: { body: 'My favourite colour is teal. Reply with "ok".',
                   context_id: ctx } });
      expect(turn1.ok()).toBeTruthy();

      const turn2 = await request.post('/api/agents/hermes-1/dispatch',
  ...
  test('aggregator does not record turns in conversation_turns for hermes-1',
    async ({ request }) => {
      const card = await waitForAgent(request, HERMES_AGENT_ID);
      test.skip(!card, 'hermes-1 not online');
      // After the previous tests sent commands, the conversations endpoint
      // should still report zero turns for hermes-1 (memory ownership rule).
      const resp = await request.get('/api/conversations?agent_id=hermes-1');
      expect(resp.ok()).toBeTruthy();
