...
def test_insert_and_fetch_returns_newest_first(fresh_db):
    db.insert_turn(context_id="ctx-1", agent_id="gemma-1",
                   role="user", content="hello", token_count=2,
                   skill_id="reasoning.chat")
    time.sleep(0.01)
    db.insert_turn(context_id="ctx-1", agent_id="gemma-1",
                   role="assistant", content="hi back", token_count=2,
                   skill_id="reasoning.chat")
...
def test_fetch_filters_by_agent_id(fresh_db):
    db.insert_turn(context_id="ctx-1", agent_id="gemma-1",
                   role="user", content="for gemma", token_count=2, skill_id=None)
    db.insert_turn(context_id="ctx-1", agent_id="ag2-1",
                   role="user", content="for ag2", token_count=2, skill_id=None)
    rows = db.fetch_recent_turns(agent_id="gemma-1", context_id="ctx-1")
    assert len(rows) == 1
...
def test_delete_turns_by_context(fresh_db):
    for ctx in ("ctx-keep", "ctx-drop"):
        db.insert_turn(context_id=ctx, agent_id="gemma-1",
                       role="user", content="x", token_count=1, skill_id=None)
...
def test_skill_id_persisted_when_provided(fresh_db):
    rid = db.insert_turn(context_id="ctx-1", agent_id="gemma-1",
...
def test_list_conversations_grouped(fresh_db):
    for role in ("user", "assistant"):
        db.insert_turn(context_id="ctx-A", agent_id="gemma-1",
                       role=role, content="x", token_count=2,
                       skill_id="reasoning.chat")
    db.insert_turn(context_id="ctx-B", agent_id="gemma-1",
                   role="user", content="y", token_count=3,
                   skill_id="text.summarize")
...
def test_list_conversations_filter_by_agent(fresh_db):
    db.insert_turn(context_id="ctx-A", agent_id="gemma-1",
                   role="user", content="x", token_count=1, skill_id=None)
    db.insert_turn(context_id="ctx-B", agent_id="ag2-1",
                   role="user", content="y", token_count=1, skill_id=None)
