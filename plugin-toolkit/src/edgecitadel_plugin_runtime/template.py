"""Shared NATS lifecycle for handler-based plugin runtimes."""
...
async def publish_log(
    ...
    *,
    ...
    Plugin runtimes call this for lifecycle events (register, shutdown) and
    ...
    try:
        await nc.publish(f"agents.{agent_id}.log", json.dumps(env).encode())
    except Exception as e:  # noqa: BLE001
        log.warning("log envelope publish failed (%s): %s", type(e).__name__, e)
...
async def main(config_path: str | Path) -> None:
    ...
    await publish_log(
        ...
        level="INFO",
        source="lifecycle",
        message=f"plugin runtime registered as {agent_id} "
    ...
    async def heartbeat():
        ...
        while True:
            ...
            await nc.publish(f"agents.{agent_id}.heartbeat", json.dumps(hb).encode())
            await asyncio.sleep(interval)

    hb_task = asyncio.create_task(heartbeat())

    pc = PullConsumer(agent_id=agent_id, nc=nc, handler=handle, ack_wait_sec=ack_wait)
    ...
    await publish_log(
        ...
        level="INFO",
        source="lifecycle",
        message="plugin runtime received shutdown signal; draining",
    )
    off = {
        "v": 1,
        "id": str(uuid.uuid4()),
