...
_CORRELATION_SCHEMA_PATH = _schema_directory() / "task-correlation.v1.json"
...
class EnvelopeValidator:
    ...
    def validate_envelope(self, doc: dict[str, object]) -> None:
        _validate(self._env, doc, "envelope")
        if doc.get("type") in CORRELATED_TYPES:
            correlated = normalize_task_correlation(doc)
            _validate(_CORRELATION_VALIDATOR, correlated, "task_correlation")

    def validate_card(self, doc: dict[str, object]) -> None:
        _validate(self._card, doc, "agent_card")

    def validate_register(self, envelope: dict[str, object]) -> None:
        ...
        if not isinstance(card, dict):
            raise ValidationError("register payload must be an Agent Card mapping")
        typed_card = cast(dict[str, object], card)
        self.validate_card(typed_card)
        if typed_card.get("name") != envelope.get("sender_id"):
            raise ValidationError(
                f"sender_id {envelope.get('sender_id')!r} must match "
                f"Agent Card name {typed_card.get('name')!r}"
            )
