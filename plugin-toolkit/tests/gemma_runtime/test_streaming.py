...
from edgecitadel_gemma_plugin.ollama_client import call_ollama_streaming
...
async def test_stream_flushes_on_token_threshold(monkeypatch):
    ...
    monkeypatch.setattr(
        "edgecitadel_gemma_plugin.ollama_client.httpx.AsyncClient",
...
async def test_stream_uses_json_format_when_requested(monkeypatch):
    ...
    await call_ollama_streaming(
        messages=[{"role": "user", "content": "x"}],
        skill={"defaults": {"temperature": 0.1}},
        args={},
        use_json=True,
        publish_progress=AsyncMock(),
    )
