...
async def test_streaming_aggregates_deltas():
    from edgecitadel_hermes_plugin.hermes_client import call_hermes_streaming

    # Stream "Hello world!" as three chunks then DONE
    body = b"".join(
        [_sse_chunk("Hello"), _sse_chunk(" world"), _sse_chunk("!"), _sse_done()]
    )
...
async def test_done_marker_terminates_stream():
    ...
    body = b"".join(
        [_sse_chunk("first"), _sse_done(), _sse_chunk("ignored")]
...
async def test_session_id_header_set_when_context_id_present():
    ...
    def _capture(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            content=_sse_chunk("ok") + _sse_done(),
            headers={"Content-Type": "text/event-stream"},
        )

    respx.post("http://localhost:8642/v1/chat/completions").mock(side_effect=_capture)
...
async def test_session_id_header_omitted_when_none():
    ...
    await call_hermes_streaming(prompt="x", session_id=None, publish_progress=publish)
