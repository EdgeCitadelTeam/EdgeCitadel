...
export default function edgecitadel(pi: ExtensionAPI) {
  ...
  pi.registerTool({ name: "edgecitadel_task_update", label: "EdgeCitadel Task Update", description: "Record an explicit task lifecycle transition.", parameters: Type.Object({ task_id: Type.String(), state: Type.Union([Type.Literal("accepted"), Type.Literal("running"), Type.Literal("completed"), Type.Literal("failed"), Type.Literal("rejected"), Type.Literal("cancelled")]), reason: Type.Optional(Type.String({ maxLength: 1024 })) }), execute: execute("edgecitadel_task_update") });
